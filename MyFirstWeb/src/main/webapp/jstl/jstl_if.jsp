...
<body>
     <!-- name이 홍길동으로 들어오는 경우만 화면에 "name
     파라미터로 들어온 값은 홍길동 입니다."라고 출력 -->
     ...
     <c:set var="fName" value="${param.name}"></c:set>
     <c:out value="당신이 입력한 값은 ${fName }입니다."></c:out><br/>
     당신이 입력한 값은 ${fName }입니다.<br/>
     
     <!-- form에서 보낸 데이터 받아서 변수로 저장하기 -->
     <c:set var="fKey" value="${param.key}"></c:set>
     <c:out value="${fName }의 키는 ${fKey }입니다."></c:out>    
</body>
</html>